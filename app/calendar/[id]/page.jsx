...
export default function CreateProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXTAUTH_URL}/calendar/new`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    async function fetchUserData() {
      try {
        const response = await fetch(`${process.env.NEXTAUTH_URL}/users/one/${id}`);
  ...
  const fetchWeights = async () => {
    try {
        const response = await fetch(`${process.env.NEXTAUTH_URL}/calendar/${id}`);
...
useEffect(() => {
  ...
  const handleWeightUpdate = async (e) => {
    e.preventDefault();
    try {
        const response = await fetch(`${process.env.NEXTAUTH_URL}/calendar/update/${currentWeight._id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
...
const handleDeleteWeight = async (weightId) => {
  try {
      const response = await fetch(`${process.env.NEXTAUTH_URL}/calendar/delete/${weightId}`, {
          method: "DELETE",
      });
